...
class Config:
    def __init__(self):
        ...
        self.render_max_workers = max(1, int(os.getenv("RENDER_MAX_WORKERS", "1")))
        ...
        self.ai_chunk_concurrency = max(1, int(os.getenv("AI_CHUNK_CONCURRENCY", "3")))
        self.ai_chunk_retry_max = max(0, int(os.getenv("AI_CHUNK_RETRY_MAX", "3")))
        self.ai_chunk_retry_backoff_base = max(0.5, float(os.getenv("AI_CHUNK_RETRY_BACKOFF_BASE", "4.0")))
        self.ai_chunk_retry_backoff_max = max(1.0, float(os.getenv("AI_CHUNK_RETRY_BACKOFF_MAX", "60.0")))
