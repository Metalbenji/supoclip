...
- Claude and other agents can access Docker through the Windows 11 host by invoking Docker CLI via `cmd.exe`.
...
uvicorn src.main_refactored:app --reload --host 0.0.0.0 --port 8181

# Legacy entrypoint (still available)
uvicorn src.main:app --reload --host 0.0.0.0 --port 8181
```
...
- Backend: `http://${APP_HOST}:${BACKEND_HOST_PORT}` (default `http://localhost:8181`, API docs at `/docs`)
...
- Backend API docs available at `http://${APP_HOST}:${BACKEND_HOST_PORT}/docs` (default `http://localhost:8181/docs`, Swagger UI)
