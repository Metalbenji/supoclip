ALTER TABLE "users"
ADD COLUMN IF NOT EXISTS "default_framing_mode" VARCHAR(32) DEFAULT 'auto',
ADD COLUMN IF NOT EXISTS "default_face_detection_mode" VARCHAR(20) DEFAULT 'balanced',
ADD COLUMN IF NOT EXISTS "default_fallback_crop_position" VARCHAR(20) DEFAULT 'center';
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_users_default_framing_mode'
  ) THEN
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_users_default_face_detection_mode'
  ) THEN
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_users_default_fallback_crop_position'
  ) THEN
